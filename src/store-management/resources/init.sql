...
(
    ID                       VARCHAR(50),
    PRODUCT_ID               VARCHAR(50),
    NOT_BILL_REMAIN_QUANTITY INT,
    CONSTRAINT PK_NOT_BILLING_PRODUCT PRIMARY KEY (ID),
    CONSTRAINT FK_NBP_PRODUCT FOREIGN KEY (PRODUCT_ID) REFERENCES PRODUCT (ID)
);
...
BEGIN
    UPDATE PRODUCT
    SET PRODUCT.TOTAL_QUANTITY = TOTAL_QUANTITY + NEW.IMPORT_QUANTITY
...
BEGIN
    ...
    DECLARE V_REMAIN_QUANTITY INT;

    UPDATE PRODUCT
    SET TOTAL_QUANTITY = TOTAL_QUANTITY - NEW.QUANTITY
    WHERE PRODUCT.ID = NEW.PRODUCT_ID;

    SELECT PRODUCT_ID, NOT_BILL_REMAIN_QUANTITY
    INTO V_PRODUCT_ID, V_REMAIN_QUANTITY
    FROM PRODUCT_EXPORT
    WHERE PRODUCT_EXPORT.PRODUCT_ID = NEW.PRODUCT_ID;

    IF V_PRODUCT_ID IS NOT NULL
    THEN
        UPDATE PRODUCT_EXPORT
        SET NOT_BILL_REMAIN_QUANTITY = NOT_BILL_REMAIN_QUANTITY + NEW.QUANTITY
        WHERE PRODUCT_ID = PRODUCT_ID;
    ELSE
        INSERT INTO PRODUCT_EXPORT VALUES (UUID(), NEW.PRODUCT_ID, NEW.QUANTITY);
    END IF;
END; //
DELIMITER ;

DROP TRIGGER IF EXISTS TRIG_INVOICE_LINE;
DELIMITER //
CREATE TRIGGER TRIG_INVOICE_LINE
    BEFORE INSERT
    ON INVOICE_LINE
    FOR EACH ROW
BEGIN
    UPDATE PRODUCT_EXPORT SET NOT_BILL_REMAIN_QUANTITY = NOT_BILL_REMAIN_QUANTITY - NEW.QUANTITY;
END; //
DELIMITER ;
